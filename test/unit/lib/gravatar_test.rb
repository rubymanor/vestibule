...
class GravatarTest < ActiveSupport::TestCase
  ...
  context "gravatar_url" do
    ...
    should "ask for the email_for_gravatar form of the user's email" do
      Gravatar.expects(:email_for_gravatar).with(@account.email).returns(@account.email)
      Gravatar.gravatar_url(@account)
    end

    should "get the md5 hash of the email" do
      Digest::MD5.expects(:hexdigest).with(Gravatar.email_for_gravatar(@account.email))
      Gravatar.gravatar_url(@account)
    end

    should "return the gravatar url for the hash of the user's email" do
      expected_url = "http://www.gravatar.com/avatar/#{Digest::MD5.hexdigest(Gravatar.email_for_gravatar(@account.email))}"
      assert_equal expected_url, Gravatar.gravatar_url(@account)
    end

    should "convert a rating option into an r query string param" do
      expected_url = /(\?|&)r=pg/
      assert_match expected_url, Gravatar.gravatar_url(@account, :rating => 'pg')
    end

    should "convert a default option into an d query string param" do
      expected_url = /(\?|&)d=404/
      assert_match expected_url, Gravatar.gravatar_url(@account, :default => '404')
    end
    ...
    should "pass default option values 404, mm, identicon, monsterid, wavatar, or retro directly as the values of a d query string param" do
      ['404', 'mm', 'identicon', 'monsterid', 'wavatar', 'retro'].each do |default_keyword_arg|
        ...
        assert_match expected_url, Gravatar.gravatar_url(@account, :default => default_keyword_arg)
      end
    end
    ...
    should "url-encode default option values not from the keyword list when passing them as the values of a d query string param" do
      ...
      assert_match expected_url, Gravatar.gravatar_url(@account, :default => "http://www.wooo.com/my_default.png")
    end
  end
end